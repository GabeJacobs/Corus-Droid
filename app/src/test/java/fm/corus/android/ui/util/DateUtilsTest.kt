...
class DateUtilsTest {
    ...
    @Test fun `under a minute reads just now`() {
        assertEquals("just now", DateUtils.relativeTime(Date()))
    }

    @Test fun `minutes use m suffix`() {
        assertEquals("5m", DateUtils.relativeTime(minutesAgo(5)))
    }

    @Test fun `hours use h suffix`() {
        assertEquals("3h", DateUtils.relativeTime(hoursAgo(3)))
    }

    @Test fun `days use d suffix`() {
        assertEquals("2d", DateUtils.relativeTime(daysAgo(2)))
    }

    @Test fun `months use mo suffix distinct from minutes`() {
        val sixtyDays = DateUtils.relativeTime(daysAgo(60))
