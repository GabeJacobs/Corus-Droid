...
import fm.corus.android.data.model.recentShareRecipients
...
/**
 * Share-sheet recents include groups and direct chats in last-message order.
 * Duplicates collapse and the grid is capped.
 */
class RankShareContactsTest {
    ...
    private fun thread(id: String, other: CymbalUser? = null, isGroup: Boolean = false) =
        CymbalThread(id = id, otherUser = other, otherUserId = other?.id ?: "", isGroup = isGroup, lastMessageAt = java.util.Date(1), lastMessageFromUserId = "me")
    ...
    fun `group threads preserve existing destinations`() {
        ...
        assertEquals(listOf("group:g1", "a", "group:g2"), result.map { it.id })
    }
    ...
    fun `result is capped`() {
        ...
        val result = recentShareRecipients(many)
